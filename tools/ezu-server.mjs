...
const
log	= ( ...a ) => console.log( '[ezu-server]', ...a )
...
callEditor	= ( method, params = {}, timeout = RPC_DEFAULT_MS ) => new Promise( ( resolve, reject ) => {
	if	( !editor || editor.destroyed ) {
		reject( new Error( 'No browser editor connected. Run npm run dev and open the editor.' ) )
		return
	}
	const
	id = randomUUID()
...
handleEditorMessage	= msg => {
	...
	case 'doc-update':
		...
		if	( msg.type === 'editor-ready' ) log( 'snapshot', lastSnapshot.watchPath, `${ msg.elements ?? 0 } elements` )
		break
	case 'rpc-result': {
...
notifySvg	= abs => {
	const
	rel = svgPathFromAbs( abs )
	if	( !rel ) return
	clearTimeout( debounce )
	debounce = setTimeout( () => {
		log( 'svg-changed', rel )
		broadcast( { type: 'svg-changed', path: rel } )
	}, 80 )
}
...
handleEzuApi	= async ( req, res, urlPath ) => {
	if	( urlPath === '/__ezu/status' && req.method === 'GET' ) {
		json( res, 200, {
			connected	: !!( editor && !editor.destroyed )
		,	watchPath	: lastSnapshot?.watchPath ?? null
		,	width		: lastSnapshot?.width ?? null
		,	height		: lastSnapshot?.height ?? null
		,	elements	: lastSnapshot?.elements ?? 0
		,	selection	: lastSnapshot?.selection ?? []
		} )
		return
	}
	if	( urlPath === '/__ezu/svg' && req.method === 'GET' ) {
		try {
			if	( editor && !editor.destroyed ) {
				const	snap = await callEditor( 'getSVG' )
				lastSnapshot = snap
				json( res, 200, snap )
			} else if	( lastSnapshot ) {
				json( res, 200, lastSnapshot )
			} else {
				json( res, 503, { error: 'No editor connected and no cached document.' } )
			}
		} catch ( er ) {
	...
	if	( urlPath === '/__ezu/rpc' && req.method === 'POST' ) {
		try {
			const
...
server = createServer( ( req, res ) => {
	...
	if	( urlPath.startsWith( '/__ezu/' ) && urlPath !== WS_PATH ) {
		handleEzuApi( req, res, urlPath ).catch( er => {
			log( 'api error', er )
